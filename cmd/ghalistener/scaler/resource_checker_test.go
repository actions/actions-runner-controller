...
func TestAdjustCount_PartialAllocation(t *testing.T) {
	...
	n, _, err := checker.AdjustCount(context.Background(), 4)
	require.NoError(t, err)
...
func TestAdjustCount_BottleneckResourceLimits(t *testing.T) {
	...
	n, _, err := checker.AdjustCount(context.Background(), 10)
	require.NoError(t, err)
...
func TestAdjustCount_NoAnnotations_ReturnsRequestedCount(t *testing.T) {
	...
	n, _, err := checker.AdjustCount(context.Background(), 5)
	require.NoError(t, err)
	assert.Equal(t, 5, n)
}
...
func TestAdjustCount_ERSFetchError(t *testing.T) {
	...
	_, _, err := checker.AdjustCount(context.Background(), 1)
	assert.Error(t, err)
}
